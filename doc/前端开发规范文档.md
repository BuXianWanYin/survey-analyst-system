...
**控制台输出规范：**
- **console.log**：生产环境禁止使用 `console.log`（`no-console` 规则为警告级别）
- **debugger**：生产环境禁止使用 `debugger`（`no-debugger` 规则为警告级别）
...
**未使用变量规范：**
- **未使用的变量**：应移除未使用的变量、导入和函数
- 开发阶段可以使用下划线前缀表示有意未使用的变量
...
// ❌ 错误
const unusedVariable = 'value' // 未使用
```
